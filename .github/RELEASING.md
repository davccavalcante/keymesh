# Releasing @takk/keymesh

This document is the runbook for publishing `@takk/keymesh` to npm and GitHub. The release flow is intentionally **two-step**: a GitHub Release is created and reviewed FIRST, and only then promoted to NPMJS.
...
```
api-key-rotation rate-limit failover load-balancing circuit-breaker openai anthropic gemini llm resilience typescript nodejs ai-infrastructure
```

And set Website: `https://keymesh.takk.ag/` (or your preferred canonical URL).
...
The workflow validates, builds, tests, packs, then creates the tag `v1.0.1` and a GitHub Release titled `[REVIEW REQUIRED — NOT YET ON NPMJS] @takk/keymesh@1.0.1`.

Visit <https://github.com/davccavalcante/keymesh/releases/tag/v1.0.1> and review:
...
The workflow verifies the Step 1 artefacts exist, validates monotonic version vs the npm registry, builds, tests, packs, then `npm publish --access public --tag <auto-resolved> --provenance`. After publish, the GitHub Release title flips to `[PUBLISHED ON NPMJS] @takk/keymesh@1.0.1`.

### 3.4 Verify the release

```bash
# Cache may take ~1 min to update; the workflow itself verifies the registry.
npm view @takk/keymesh versions
npm view @takk/keymesh dist-tags
...
# Verify provenance
npm view @takk/keymesh@1.0.1 --json | jq .dist.attestations
```
...
```bash
npm dist-tag add @takk/keymesh@1.1.0-rc.1 latest
```
...
```bash
npm unpublish @takk/keymesh@1.0.1
```

After 72 hours, unpublishing is not permitted. Use `npm deprecate`:

```bash
npm deprecate @takk/keymesh@1.0.1 "Replaced by 1.0.2 - fixes circuit-breaker race."
```
...
| Promote a prerelease to latest | `npm dist-tag add @takk/keymesh@<semver> latest` |
| List versions | `npm view @takk/keymesh versions` |
| Verify provenance | `npm view @takk/keymesh@<semver> --json \| jq .dist.attestations` |
